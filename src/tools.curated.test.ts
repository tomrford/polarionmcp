...
describe("curated tools", () => {
  ...
  test("list_linked_work_items returns outgoing links", async () => {
    getSpy.mockResolvedValueOnce({
      data: {
        data: [{ id: "PRJ/REQ-1/relates_to/PRJ/REQ-2", type: "linkedworkitems", attributes: { role: "relates_to" } }],
        meta: { totalCount: 1 },
        links: {},
      },
      response: { ok: true, status: 200 },
    ...
    expect(textPayload(result)).toEqual({
      items: [{ id: "PRJ/REQ-1/relates_to/PRJ/REQ-2", type: "linkedworkitems", attributes: { role: "relates_to" } }],
      pagination: {
        total: 1,
        page_size: 20,
        page_number: 1,
