...
import { errorResult, ok, authHeaders, interpolatePath, toQueryString, truncateResponse } from "../helpers.ts";
import { makeError, networkError } from "../errors.ts";
import {
  getResolvedReadOperations,
  resolveReadOperation,
} from "../openapi/read-policy.ts";
...
function validateQueryParams(
  ...
  if (invalid.length > 0) {
    throw new Error(
      `Unsupported query parameter(s): ${invalid.join(", ")}. Allowed: ${allowedQueryParamNames.join(", ") || "none"}`,
    );
  }
...
export function buildReadUrl(
  ...
  const finalQuery = {
    ...(validatedQuery ?? {}),
    ...(catalogEntry.queryParamNames.includes("page[size]")
      ? { "page[size]": page_size }
      : {}),
    ...
    withToolLogging(
      ...
      ({
        ...
        return {
          ...
          page_number,
          policy_mode:
            typeof parsed?.["policy_mode"] === "string"
              ? parsed["policy_mode"]
              : undefined,
          truncated:
            !!parsed &&
