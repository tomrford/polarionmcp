...
export function helpSearch(
    ...
    const byResource = Object.entries(
      all
        ...
        .reduce<Record<string, number>>((acc, entry) => {
          acc[entry.catalogEntry.resourceGroup] =
            (acc[entry.catalogEntry.resourceGroup] ?? 0) + 1;
          return acc;
        }, {}),
    )
  ...
  return {
    summary: {
      total_matches:
        curated.length + generic_read.length + advanced.length + blocked.length,
      filters: {
        keyword,
        resource_type,
        include_blocked,
...
export function registerApiHelpTool(server: McpServer) {
  server.registerTool(
    "polarion_api_help",
    {
      ...
      description:
        "Discover curated and generic read operations by keyword or resource type.",
      inputSchema: {
        keyword: z
          .string()
          .optional()
